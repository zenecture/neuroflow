...
package object cuda extends Logs {
  ...
  def allocate[V:ClassTag](size: Long) = {
    ...
    if (!hasFreeMemory(size * io.getTargetSize)) throw new OutOfMemoryError(s"CUDA Memory")
  ...
  implicit class enrichBridjPtr[T](val pointer: Pointer[T]) extends AnyVal {
    def toCuPointer = {
      assert(pointer != null, f"GPU Garbage Collection Race Condition. Fine tune neuroflow.core.Settings.gcThreshold for your GPU. Current GcThreshold: ${ GcThreshold() / 1024.0 / 1024.0 }%.6g MB")
