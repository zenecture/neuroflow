...
object CuMatrix extends LowPriorityNativeMatrix with CuMatrixOps with CuMatrixSliceOps with CuMatrixFuns {
  ...
  def fromDense[V](mat: DenseMatrix[V])(implicit ct: ClassTag[V]): CuMatrix[V] = {
...
trait CuMatrixFuns extends CuMatrixKernels { this: CuMatrix.type =>
  ...
  object Activators {
    ...
    def square[T](implicit broker: KernelBroker[T])             =  broker.implFor[Square[T]]("square_a")
    def square_derivative[T](implicit broker: KernelBroker[T])  =  broker.implFor[Square[T]]("square_d")
