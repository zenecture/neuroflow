...
/**
  ...
  * If `stop` is set to true, the training will terminate if both errors diverge.
  */
case class EarlyStopping(stop: Boolean, testSet: Seq[Seq[Double]]) extends Regularization
