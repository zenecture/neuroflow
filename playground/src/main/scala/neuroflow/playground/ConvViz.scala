...
object ConvViz {
  ...
  def apply = {
    ...
    val c0 = Convolution(dimIn = (400, 400, 3), padding = 1`²`, field = 3`²`, stride = 1`²`, filters = 3, activator = f)
    val c1 = Convolution(dimIn = c0.dimOut,     padding = 1`²`, field = 3`²`, stride = 1`²`, filters = 3, activator = f)
    val c2 = Convolution(dimIn = c1.dimOut,     padding = 1`²`, field = 4`²`, stride = 2`²`, filters = 3, activator = f)
    val c3 = Convolution(dimIn = c2.dimOut,     padding = 1`²`, field = 3`²`, stride = 1`²`, filters = 3, activator = f)
    ...
    samples.zipWithIndex.foreach {
      case ((xs, ys), i) =>
        ...
        i0s.zipWithIndex.foreach {case (img, idx) => Image.writeImage(img, path + s"/$i-c0-$idx.png", PNG) }
        i1s.zipWithIndex.foreach {case (img, idx) => Image.writeImage(img, path + s"/$i-c1-$idx.png", PNG) }
        i2s.zipWithIndex.foreach {case (img, idx) => Image.writeImage(img, path + s"/$i-c2-$idx.png", PNG) }
        i3s.zipWithIndex.foreach {case (img, idx) => Image.writeImage(img, path + s"/$i-c3-$idx.png", PNG) }
    }
